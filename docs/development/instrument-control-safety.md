...
SEQ 正常完成、Stop、Error 或任务取消都不会调用 `hold()`，也不会向温度或磁场发送新目标。
因此 Stop 时正在斜坡的仪表会继续执行仪表中已有的目标和速率。操作者明确点击
`Hold Current` 时，若后端通过“把当前值设为新目标”实现 Hold，必须在同一次操作里读取
真实当前值并验证，再写一次；不能使用几秒前的缓存、猜测值或零。

`close()` 只负责释放通讯资源，不应擅自关闭输出或改变设定。具体安全状态应由仪表本机
保护、硬件联锁、明确的手动操作和已注册事件响应共同保证。

## 事件响应不能直接操作另一台后端

需要“某个报警触发磁场归零”时，由报警来源的 System Instrument 在
`event_responses()` 中返回 `EventResponseSpec`。核心把来源绑定到当前逻辑仪表，匹配
`code + context`，再通过目标仪表现有的 `set_target()` 路径执行 `zero`。这样仍保留单仪表
串行、上下限、写超时不重放和进程隔离。

响应的优先级高于控制、测量即时读取和后台状态读取，但不会拆断一条已经开始的完整仪表
事务。第一次事件执行响应并锁存目标；重复事件只更新计数。响应锁存期间普通 Set、Hold 和
系统指令都被拒绝，报警前已排队但尚未开始的写入也会在执行前再次检查锁。源事件解除、
响应动作完成后还要人工复位。Error 仍中止 SEQ，Warning 仍继续；SEQ
自身的退出路径不会控制 System Instrument。不要从后端保存核心对象、另一台仪表对象或
运行任意回调。
