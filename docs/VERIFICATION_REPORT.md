...
0.7.0 仿真框架达到本阶段交付条件：英文主界面、`2nd Stage` 只读辅助温度、SEQ Ctrl/Shift 多行选择和批量右键/键盘编辑、T/F 命令禁用、Data Browser 一次确认式多 Y 选择、X/Y 独立 Log10 与 `.plt` 显示恢复、源码与打包程序均可启动；多层 SEQ 能完成；数据与事件日志可生成；Warning/Error、判稳、安全限制和中止保持行为均有自动测试覆盖。
...
- Windows 发布 EXE 独立完成同一流程，退出码为 0。
- 人工检查发布 EXE 截图：英文菜单/工具栏、左侧 Sequence Control、中间单行 SEQ、右侧命令栏以及底部温度/磁场/测量卡片均正常显示。
...
Windows 包与源码包均重新解压到全新目录后验证，没有借用源码目录中的模块或配置：

- 解压后的 Windows EXE 运行完整无界面嵌套序列，退出码 0。
- EXE 运行带普通禁用行和禁用 Scan 子树的示例，退出码 0；跳过事件存在，运行快照保留 2 条 `F` 指令行。
- EXE 打开用户 DAT 及示例 `.plt`，三幅共享 X 子图正常渲染，GUI 冒烟退出码 0。
- 把同一 DAT 复制为没有对应 PLT 的新文件后由 EXE 打开，自动生成同名 `.plt`，GUI 冒烟退出码 0。
- 解压后的源码包使用隔离 Python 环境运行全部 33 项测试，退出码 0。
