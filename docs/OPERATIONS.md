...
温度和磁场的 Set/Scan 参数窗口会在底部显示当前 `device_id` 的 Configured limits。Target、Start、Stop 和 Rate 输入范围直接来自配置文件；磁场切换 Oe/T 时，当前值和限制一起换算。若输入未知或类型不匹配的 `device_id`，窗口会提示没有匹配的配置限制，运行前的核心安全校验仍然保留。
...
List 中任一空项、非数字、非有限值或超出当前设备配置上下限的点都会在参数窗口中提示并阻止确认。运行前还会再次检查全部点和速率；有一项越界就不会开始该 Scan。
