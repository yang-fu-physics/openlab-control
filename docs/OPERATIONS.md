...
- 底部 `Instrument Status`：Temperature、Magnetic Field、`2nd Stage` 等控制/Monitor；同一
  仪表返回的附加读数会作为同样大小的只读卡依次排在右侧，不会把主卡向下撑大。卡片使用
  固定浅色配色，不跟随 Windows 深色主题变色；不再显示测量 Transport 块。
- 工具栏 `Modules`：测量模块管理。
