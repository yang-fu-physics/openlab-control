...
- 恢复 `controller` 面板的粗体标题、大号当前值和紧凑三层布局；修正通用面板对象名与主窗口
  字号样式不一致导致的普通字重、小字号和大片空白。
- 将只读显示明确分为两种清单模板：`readout` 只显示一个主读数，`readout_grid` 直接以
  2×2 显示最多四个读数，超过四个继续放在右侧；两者都不再重复显示外层标题、Monitoring
  状态或灰色内框。控制和开关面板的辅助读数继续使用相同网格。
- README、运维文档、System Instrument 教程和示例仓库说明已同步；清单校验正式接受
  `readout_grid`。源码完整测试通过 285 项，严格文档构建通过。
- Windows 资产继续由 GitHub Actions 构建，并检查目录结构、ZIP 内容和 SHA-256；按本次发布
  要求不再自动启动打包后的两个 EXE，实际 EXE 行为由下载后的独立测试确认。
