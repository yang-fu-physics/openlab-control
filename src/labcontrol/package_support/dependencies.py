"""System Instrument 与 Measurement Module 额外依赖的离线验证与隔离安装。

主框架锁定并提供 PySide6、PyVISA、packaging 等共同依赖。各自的 ``requirements.lock`` 只
允许声明额外包，并从本地 wheel 目录安装到“类型 + ID + 内容指纹”对应的私有 site-packages。
...
from .trust import ContentTrustError, content_tree_digest


_HASH = re.compile(r"--hash=sha256:([0-9a-fA-F]{64})(?:\s|$)")

# 这些是 System Instrument 和 Measurement Module 可以直接使用、并由 OpenLab Control
# 统一锁定和随正式包携带的依赖。即使清单声明了兼容范围，也不会再创建隔离副本。
...
def partition_package_dependencies(
    dependencies: Iterable[str],
) -> tuple[tuple[str, ...], tuple[str, ...], tuple[str, ...]]:
    """把外部内容依赖分成核心已提供和仍需隔离安装的两组。

    返回 ``(framework, extra, errors)``。核心依赖使用全局锁定版本，所有模块和仪表
    后端看到的是同一份包；只有 ``extra`` 才要求对应目录携带 requirements.lock/wheels，
    并在 Install Dependencies 后加入对应 worker 的 sys.path。

    manifest 可以继续声明 PyVISA 等核心包的兼容范围，便于表达最低要求。若其范围不
    接受核心锁定版本，对应内容会在导入源码前被判为不兼容，不能用私有副本覆盖核心版本。
    """
...
def validate_requirements_lock(
    ...
    if not declared:
        return ()
    path = package_directory / "requirements.lock"
    if not path.is_file():
        return (
...
def dependency_runtime_errors(
    ...
    try:
        ...
        return tuple(dict.fromkeys(errors))
    try:
        current = content_tree_digest(site_packages)
...
def install_offline_dependencies(
    *,
    ...
    command.extend(
        ["-r", str(package_directory / "requirements.lock")]
    )
    creationflags = (
    ...
    try:
        ...
        marker = {
            ...
            "runtime_digest": content_tree_digest(
                staging_site
            ),
        }
