...
- Measurement Module 需要温场值时会即时采样，不复用最多一个前面板刷新周期以前的缓存；
  同一时刻多个模块请求会合并。Device Plugin 可让该测量快照只查询主值，完整安全状态
  仍由常规控制轮询持续检查。同一设备不会并发访问；当前完整仪表事务结束后，测量读取
  优先于尚未开始的后台轮询，控制与安全操作仍排在最前。
- 空闲前面板默认一秒读取一次；SEQ 控制期间内部以较短周期判稳，但前面板和 Live Trend
  仍保持一秒更新，不会跟着内部采样频率快速刷新。
